...
`~/dev/tinshell/` is a multi-app TINSHELL home structured as a **monorepo with
...
├── systemd/         canonical unit TEMPLATES (__HOME__ substituted on install)
...
| `common/config/app-store` | `createAppStore(appName, opts?)` — per-app config facade over `createConfigStore` (dir `~/dev/tinshell/apps/<name>`): get/all/set (schema-checked, `{ok,error}`)/reloadConfig; `opts.onReject` overrides the rejection sink (media logs via `common/log/logger`). Non-surface apps' `config.ts` are thin re-exports of one instance |
...
- the shared-group unix socket `/run/ags/applets.sock`
...
machine-level intent file (`/var/lib/ags/charge-cap`), both through scoped
...
drift apart; the entries are `__HOME__` templates like the systemd unit
templates, with the token substituted for the real home at install time by
...
(the `ags-greeter` group + the `/run/ags` tmpfiles.d entry the pre-login greeter
...
| --- | --- | --- |
| LIVE member of a defaults+live pair (e.g. `/etc/greetd/ags-greeter/config.json`) | seeded when ABSENT; PRESERVED when it exists — the loader merges `config.defaults.json` UNDER the live file, so the overwrite buys nothing | `deploy_seed_config <src> <dst>` |
...
- **This home is NOT a git repo** — destructive changes are unrecoverable:
  `cp -r ~/dev/tinshell /tmp/tinshell-backup-$(date +%F)` (or `git init`) before any
  structural or delete edit.
...
`/etc/greetd/ags-greeter.sh` plus its `.stamp.json`) still carries its own
