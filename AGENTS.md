...
| `common/config/loader` | `createConfigStore(dir)` — schema-driven three-file (defaults+schema+live) loader: atomic reload, dotted get/set, deepMerge, serialized write chain, `onConfigChanged`, tier metadata. `setDottedPath(root, path, value)` — the ONE dotted-path setter (creates intermediate objects, `false` when an intermediate is a non-object): `setLive` walks the live tree through it, and so does the dock's detached-clone batch commit (`apps/dock/config-clone.ts` — `safeClone`, the detached copy the dock stages before `dock.queueWrite`, shared by the move-mode snap persist and the `config set|update` batch commit). `setLive` is the ONE live-mutation primitive and ANNOUNCES what it mutates: it fires the store's change listeners when the value at the path actually differs, so every set path built on it (the facade's `set`/`setLive`, app-store's `set`, and therefore every `<app> config set` command) reaches the app's `onConfigChanged` mirrors without a restart, and a write of the value already there announces nothing. Also `appConfigDir(name)` → `apps/<name>` |
| `common/config/facade` | `createConfigFacade(store)` — the app-facing store wrapper (stable `config` mirror, filtered onConfigChanged, validated set/applyToLive/reload; a `set` that changes the value fires the filtered listeners, since the store announces it). Every app's `config.ts` owns its store; no module knows the surface list |
...
| `scripts/palette/**` | the palette wiring this repository owns: `theme.template.ts` and `tokens.template.ts` (the templates that render the two carriers), their committed `*.record.json` records, and `pin.json` — the palette repository, revision and `palette.json` digest this tree was rendered against. See "Generated carriers and the palette gate" |
| `scripts/check-palette.mjs` | `npm run check:palette` — the palette gate: re-renders both carriers through the palette's own renderer and fails on a stale carrier, a stale record, or a palette that is not the pinned digest (`--write` re-renders instead of comparing). Part of `npm run check`; CI runs it as the `palette` job |
...
| `common/css/tokens` | the TS half of the design-token layer — the runtime carrier of a token whose CSS carrier is the `--tinshell-*` custom property in `common/shell/theme.css`, for consumers that read no stylesheet (a Cairo painter, a `Pango.FontDescription`, a config fallback string). `FONT_FAMILY`, `INK`, `INK_MUTED`, `ACCENT`; a token the stylesheet alone can serve (every surface or geometry token below) has no constant here. **GENERATED**: the colour constants are rendered from the house palette by `scripts/palette/tokens.template.ts`, so this module and the stylesheet carry the palette's values from one render and cannot disagree (`FONT_FAMILY` is this repository's own value and rides along). A token is named for the ROLE the value plays, never for the value: two apps read one role (the suite accent, the muted ink), while a value two sites happen to share can be two different decisions (media's white seek-slider fill is a control fill, not label ink) and stays spelled at the site. Those two files are the only ones allowed to spell a token's value — the audit's `literal-duplicated` class allowlists the pair, so a re-spelling elsewhere is reported. Edit the template, then `npm run check:palette -- --write`; the file, its template and its record are committed together, and the gate fails when they drift |
...
| `common/scroll` | the ROW-SCROLL laws every vertical scroller in the suite scrolls by, in one pure (gi-free) module: the wheel/selection laws (`wheelSteps` — a notch is whole row units, a fractional click none — `stepSelection`, `offsetForSelection`), the two conversions between a scroller's adjustment and a surface's own row unit (`offsetPixels` out, `rowOffset` for a position READ BACK from a scroller that moves its own adjustment), the viewport bound (`viewportRows`, `viewportPixels` — the `max-content-height` cap that binds only while the scroller's VERTICAL policy is `AUTOMATIC`), the selection-follow (`selectionInView`), the decision for ONE `Gtk.EventControllerScroll` event (`scrollDecision(unit, dy, gridActive)` → a whole-row `step` / `continuous` / `ignore`, the `gridActive` rule being the surface whose inner scroller owns the event) and `SCROLL_CONTROLLER_FLAGS` (`VERTICAL` alone: `DISCRETE` would make `get_unit()` answer WHEEL for every trackpad delta, and `KINETIC` only gates a `::decelerate` nobody takes — the momentum is the widget's own), which carries the ONE ownership rule: a surface leaving its gesture to a `GtkScrolledWindow` must NOT consume a continuous delta, because `gtk_widget_run_controllers` stops the dispatch at the first non-gesture controller returning TRUE — the scroller's own handler then never runs and GTK's kinetic scrolling (`GtkKineticScrolling`, its own friction curve with end overshoot) is latched off for the whole gesture. No surface in the suite carries an app-side tail, decay curve or position accumulator: a surface keeps only what a DISCRETE wheel notch must do (the launcher steps its selection, its glyph grid steps its view) and hands every continuous delta on. A consumer counts rows in its OWN unit (the result list entries, the emoji glyph grid grid rows); `common/scroll.probe.ts` pins the laws — the wheel step, the read-back of a position a scroller moved, the viewport cap — without a window |
...
| `common/shell/theme.css` | **GENERATED from the house palette** by `scripts/palette/theme.template.ts` — shared CSS primitives every app imports FIRST: the `--tinshell-*` token block declared on `*`, so every widget carries the value itself (GTK custom properties are inherited and GTK has no document element for a browser's `:root`) — `--tinshell-font-family`, the ink tokens (`--tinshell-ink`, `--tinshell-ink-emphasis`, `--tinshell-ink-muted`, `--tinshell-ink-faint`, `--tinshell-accent`), the surface tokens (`--tinshell-panel` — the palette's `surface.base` painted at its `opacity.panel` — `--tinshell-wash`, `--tinshell-row-selected`, `--tinshell-hairline`), the scrollbar pill's states (`--tinshell-scrollbar-thumb`, `--tinshell-scrollbar-thumb-hover`, `--tinshell-scrollbar-thumb-active`) and the geometry/type tokens (`--tinshell-panel-radius`, `--tinshell-row-radius`, `--tinshell-row-padding`, `--tinshell-font-size-body`). Every colour token is the PALETTE's value; the font stack and the geometry tokens are this repository's own. A `var()` resolves across providers, so a per-app sheet added at its own priority consumes them. Then the `entry` base and the thin-pill scrollbar family (reading the scrollbar tokens). A token a consumer CSS cannot reach (a Cairo painter, a `Pango.FontDescription`, a config fallback string) has its second carrier in `common/css/tokens`. The stylesheet is generated WHOLE because GTK CSS has no `@import` and every app reads it as a bundled text module, so a separate palette fragment could not reach a widget without every app concatenating two stylesheets — edit the template, then `npm run check:palette -- --write` |
...
agent silently replaces the first), the portal impl name.
...
disables the packaged daemon that would race a registration slot
(`hyprpolkitagent.service` for the polkit agent slot),
...
- **Never staged:** build artifacts and bundle caches (`dist/`, `*-tinshell.js`,
  ...
  `~/.local/state/tinshell/`. Two generated kinds are the exception — the
  committed, regenerated, never hand-edited `apps/<app>/config.schema.json` and
  the palette carriers with their records (`common/shell/theme.css`,
  `common/css/tokens.ts`, `scripts/palette/*.record.json`; see §Generated
  carriers and the palette gate).
...
|---|---|---|
| `common/shell/theme.css` | `scripts/palette/theme.template.ts` | `scripts/palette/theme.record.json` |
| `common/css/tokens.ts` | `scripts/palette/tokens.template.ts` | `scripts/palette/tokens.record.json` |

- **The carriers ARE the generated files; nothing sits beside them.** Those two
  paths are the ones the audit's `literal-duplicated` allowlist names and the
  ones every app already imports, so the palette reaches the widgets through the
  existing import graph. GTK CSS has no `@import` (the parser carries no such
  at-rule) and the stylesheet arrives as a bundled text module, so a separate
...
- **The record is the other half of the pin.** Each record holds three content
  ...
  `record-stale`, naming the field that moved.
- **`npm run check:palette`** (`scripts/check-palette.mjs`) is the gate:
  `--write` re-renders both carriers and their records, the bare form re-renders
  in memory and compares. Exit 0 current, 1 drift, 2 the render could not happen
  (no palette checkout, an unreadable pin, a template that throws) — 2 is never
  read as current. CI runs it as the `palette` job against the palette cloned at
  the pinned revision.
- **A carrier is committed, never hand-edited.** Like
  `apps/<app>/config.schema.json` it is tracked and regenerated: a change made in
  the file itself fails the gate and the next re-render discards it.
- **The activation step is the next build.** Both carriers live under `common/`,
  which is the bundle's source set (`bundle_source_dirs`), so a re-rendered
