...
echo "  3. apps/$NAME/identity.ts (the namespace/app_id the surface passes) + apps/$NAME/hypr-rules.ts (its blur layer rule, or a float window rule for desktop windows) + an entry in the rule generator's ORDER table; then npm run gen:hypr-rules."
